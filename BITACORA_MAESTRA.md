...
*Última actualización: 2026-07-19 08:30:00 UTC*
...
Estado actual: **BUG-AI-002** (cuentas contables IA) en PR — prompt CB arrendador + post-proceso `normalizeInvoiceAiResponse` (ingreso inquilino → 705, no 629/628). Pendiente merge + redeploy. OpenRouter free VL: rama/PR modelos vivos aún por mergear si no está en `dev`. Rotar keys expuestas en chat.
...
*   **Log de Acciones:**
    - `[00:13:00]` - **FIX:** `utils/invoiceDedup.ts` — `computeFileSha256` delega en `bankStatementFingerprint` (guarda SubtleCrypto).
    - `[00:13:30]` - **TEST:** Casos feliz + fallo sin `crypto.subtle` en `invoiceDedup.test.ts`.
    - `[00:13:58]` - **DOC:** Registro `BUG-DEDUP-001` en bitácora.
*   **Resultado:** El TypeError opaco se sustituye por error controlado; mismo hash compartido con extractos.
