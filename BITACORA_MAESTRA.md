...
*   **Log de Acciones:**
    ...
    - `[21:36:00]` - **FIX:** `services/appwriteService.ts`. **CAMBIOS:** (1) `ID.unique()` generado antes del lambda para suppliers y apartments: el reintento usa el mismo ID → Appwrite retorna 409 (no reintentable) → sin duplicación. (2) Guardia de idempotencia añadida al inicio de cada sección: si el ejercicio destino ya tiene proveedores/apartamentos, se omite la copia y se devuelve el conteo existente.
