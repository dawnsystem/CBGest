...
*   **Log de Acciones:**
    ...
    - `[20:55:00]` - **FIX:** `App.tsx`. **CAMBIOS:** (1) Eliminado `setIsDataLayerInitialized(true)` de la línea 196 (antes del trabajo asíncrono). (2) Añadido `setIsDataLayerInitialized(true)` en el bloque `finally` del fetch inicial dentro de `initDataLayer()`. (3) Añadido `setIsDataLayerInitialized(true)` en el branch `else` (no-APPWRITE), con comentario explicativo de BUG-022. Los dos fetches ahora son secuenciales: `fetchForYear` solo puede iniciarse DESPUÉS de que `initDataLayer` haya completado (o fallado) su carga.
    ...
    - `[21:02:00]` - **COMMIT:** `fix(app): prevent race condition showing wrong fiscal year data (BUG-022)` + `fix(app): add BUG-022 comment on non-APPWRITE initialization path` pusheados al PR.
